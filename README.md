...
| `recommend.html` / `recommend.css` / `recommend.js` | **پیشنهادهای هوشمند**: فید شخصی‌سازی‌شده («چون تماشا کردی»، «مشابه …»، «مخصوص تو») با توضیحِ علتِ هر پیشنهاد؛ برای مهمان‌ها هم به‌صورت «محبوب‌ترین‌ها» کار می‌کند |
...
| `data/schedule.js` | **منبع واحد تقویم پخش** (`window.NEON_SCHEDULE_DATA`) — هم ویجت صفحه اصلی و هم صفحه‌ی تقویم از همین می‌خوانند تا هرگز اختلاف نداشته باشند |
| `assets/vendor/` | نسخه‌ی خودمیزبان `gsap` و `typed.js` (بدون وابستگی به CDN) |
...
| `bottom-nav.js` | **نوار ناوبری پایین موبایل (منبع واحد)**: همان ۵ تب در همه‌ی صفحات (`خانه / آرشیو / جستجو / ویژه / پروفایل`) با آیکون‌های یکسان؛ تب فعال از روی صفحه‌ی جاری (یا `data-active` روی `<nav class="mobile-bottom-nav">`) تعیین می‌شود و هرگز دو تب هم‌زمان فعال نمی‌شوند. «جستجو» و «ویژه» مودال همان صفحه را باز می‌کنند و در نبودِ آن به `catalog.html#search` و `subscribe.html` می‌روند |
...
| `assets/` | تصاویر WebP، favicon، آیکون‌های PWA، og-image، فونت‌ها و کتابخانه‌های `vendor` |
| `manifest.webmanifest`, `robots.txt`, `sitemap.xml` | متادیتای سایت |
| `tests/run-tests.js` | تست‌های خودکار بدون مرورگر (با `jsdom`): اسکن همه‌ی صفحات + شبیه‌سازی جریان‌های کاربر — اجرا با `npm test` |
...
### 🧪 تست خودکار
یک مجموعه تست بدون مرورگر (با `jsdom`) وجود دارد که همه‌ی صفحات را لود و جریان‌های اصلی کاربر را شبیه‌سازی می‌کند:
...
این تست‌ها در هر پول‌ریکوئست در گیت‌هاب‌اکشنز (`.github/workflows/ci.yml`) هم اجرا می‌شوند.

### 📲 PWA (نصب + آفلاین)
سایت یک **PWA** است: `manifest.webmanifest` + `sw.js` + `sw-register.js` (که در انتهای همه‌ی صفحات لود می‌شود). یعنی:
- روی موبایل/دسکتاپ قابل «نصب» است (Add to Home Screen) و مثل یک اپ جدا باز می‌شود.
- صفحاتی که یک‌بار دیده شده‌اند، آفلاین هم در دسترس‌اند (استراتژی network-first برای HTML و stale-while-revalidate برای استاتیک‌ها؛ یعنی آپدیت‌های جدید همیشه دیده می‌شوند).
...
8. صفحه‌ی «پیشنهادهای هوشمند» (`recommend.html`) که لینک‌های مرده‌ی «✨ پیشنهاد هوش مصنوعی» را به یک موتور پیشنهاد واقعی (روی تاریخچه/لیست/امتیازها) وصل می‌کند
9. PWA: `sw.js` برای کار آفلاین و قابلیت نصب + اصلاح `manifest.webmanifest` (scope/start_url)
10. اصلاحات ریز: لینک‌های مرده‌ی فوتر (قوانین/حریم خصوصی/تماس/اشتراک ویژه)، تایپوهای داده‌ها، پارامتر `?restart=1` در پلیر (شروع از اول)، حفظ تم در خروج از حساب، `h1` صفحه‌ی ورود

جزئیات و آنچه هنوز مانده (بک‌اند، پرداخت، HLS، ادمین): `CODE-REVIEW.md`

## 🚀 ارتقای «Neon Bar» (نسخه‌ی v5)
نوار پایین موبایل، آپدیت خودکار و آفلاین — رفع کامل باگ «روی آرشیو که می‌زنی آیکون‌ها عوض می‌شن و یکی کم می‌شه»:

1. **نوار پایین v2**: قرارداد ۵ تبه (خانه/آرشیو/جستجو/ویژه/پروفایل) دست‌نخورده ماند؛ فقط قشنگ‌تر شد — «پیل» نئونیِ لغزان پشت تب فعال (RTL-safe)، پاپِ فنری آیکون فعال، موج (ripple) و لرزش هپتیک روی لمس، هاله‌ی درخشان تب ویژه، جمع‌شونده هنگام اسکرول به پایین. همه‌ی حرکت‌ها با `prefers-reduced-motion` خاموش می‌شوند.
2. **شفای فایل کهنه (علت اصلی باگ)**: `sw-register.js` حالا با `updateViaCache:'none'` ثبت می‌کند، هر ۳۰ دقیقه و با هر بازگشت به تب، آپدیت را چک می‌کند و وقتی SW جدید فعال شد، صفحه را **یک‌بار** با toast بازخوانی می‌کند — بازدیدکننده‌ی قدیمی دیگر هرگز روی HTML/CSS/JS کش‌شده‌ی نسخه‌ی قبلی گیر نمی‌کند.
3. **آفلاین کامل**: `sw.js` → کش v5؛ ۶ صفحه (کتابخانه، ورود، قوانین)، ۵ اسکریپت، `pages.css` و هر دو فونت وب که از precache جا مانده بودند اضافه شدند — قبلاً همین‌ها آفلاین می‌شکستند. تست جدید جلوی تکرارش را برای همیشه می‌گیرد.
4. **گذار سینمایی بین صفحات** با View Transitions API (فقط جلوه؛ در مرورگرهای قدیمی بی‌اثر).
5. **PWA**: میان‌بُرهای سریع «آرشیو / تقویم پخش / پروفایل» در manifest.
6. **تست‌ها**: ۴ تست تازه — idempotent بودن رندر نوار (دیگر هیچ‌وقت تب کم/زیاد نمی‌شود)، decorated بودن پیل/ریپل برای اسکرین‌ریدرها، کامل بودن precache و سالم بودن جریان آپدیت. `npm test` → ۲۳/۲۳ ✓
